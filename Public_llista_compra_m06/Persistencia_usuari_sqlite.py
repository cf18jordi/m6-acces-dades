...
class Persistencia_usuari_sqlite(Persistencia_usuari.Persistencia_usuari):
    ...
    def get(self, id):
        ...
        if len(registres) > 0:
            ...
            resultat = Usuari.Usuari(
                self.configurador.get_Persistencia_factory().get_Persistencia_usuari_factory(), id, usuari_nom,
    ...
    def get_from_apikey(self, id_sessio):
        db = self.__get_db_connection()
        query = "select id, usuaris.usuari, password_hash from usuaris inner join sessions_usuaris on " \
                "(sessions_usuaris.usuari = usuaris.id) where sessions_usuaris.uuid = %s;"
        valors = (id_sessio,)
        cursor = db.cursor()
        cursor.execute(query, valors)
        ...
        if len(registres) > 0:
            ...
            resultat = Usuari.Usuari(self.configurador.get_Persistencia_factory().get_Persistencia_usuari_factory(),
                                     usuari_id, usuari_nom, usuari_password)
            return resultat
        return None
